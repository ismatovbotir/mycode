...
<div class="p-6">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Add Integration Field</h1>
        <p class="text-gray-600 text-sm mt-1">Define a new credential field for integration setup</p>
    </div>

    <div class="bg-white rounded-lg shadow border border-gray-200 p-6 max-w-2xl">
        <form method="POST" action="{{ route('admin.integration-fields.store') }}" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                >
                    ...
                    @foreach($types as $key => $label)
                        <option value="{{ $key }}" @selected(old('integration_type') === $key)>{{ $label }}</option>
                    @endforeach
                </select>
